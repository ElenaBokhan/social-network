...
const createField = (component: InputType | string, 
					 type: string, 
					 name: string, 
					 validate: Array<validatorsType> | null, 
					 props = {} ) => {
	...
	return 	<div className = {loginStyles.container}>
					<div className = {loginStyles.loginBlock}>
						<form  onSubmit={handleSubmit}>
							...
							{ createField(Input, "text", "login", [emptyField]) }							
							<p className = {loginStyles.title}>PASSWORD</p>
							{ createField(Input, "password", "password", [emptyField]) }							
							{ createField("input", "checkbox", "rememberMe", null, {className:loginStyles.checkbox}) }							
