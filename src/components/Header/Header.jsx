		...
		return 	<div className = {headStyles.header}>
					<div className = {headStyles.menu}>
						...
						<div className = {headStyles.authBlock}>
							{this.props.isAuth ? <>	<div className = {headStyles.avatar} style = {{backgroundImage: `url(${this.props.smallPhoto})`}}></div>
												<span className = {headStyles.name}>{this.props.name}</span>
												<button className = {headStyles.exit}>
													<svg 	className = {headStyles.exitIcon}
															onClick = { this.props.logoutUserThunkCreator }
